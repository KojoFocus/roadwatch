...
<p style="margin-top:40px;font-size:10px;color:#ccc;">RoadWatch Ghana · This document is for official use.</p>
</body></html>`);
  win.document.close();
  win.print();
}
...
// ─── DETAIL PANEL ─────────────────────────────────────────────────────────────
function DetailPanel({ r, onClose, onUpdate }: { r:any; onClose:()=>void; onUpdate:(u:any)=>void }) {
  ...
  useEffect(()=>{
    fetch(`/api/reports/${r.id}`).then(res=>res.json()).then(j=>{ if(j.success) setActs(j.data.activities||[]); }).catch(()=>{});
  },[r.id]);
  ...
  const save = async () => {
    ...
    } catch { onUpdate({...r,status,resolutionNote:note,fixedBy}); setSaved(true); setTimeout(onClose,700); }
    setSaving(false);
  };
        ...
        <div style={{padding:"14px 18px 44px"}}>
          ...
          <div style={{marginBottom:12}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
              {Object.entries(ST).map(([key,m]:any)=>(
                <button key={key} onClick={()=>setStatus(key)} style={{background:status===key?m.bg:"#0C0C0C",border:`1px solid ${status===key?m.b:"#141414"}`,borderRadius:11,padding:"10px 12px",color:status===key?m.c:"#2a2a2a",fontSize:12,fontWeight:700,fontFamily:"inherit",textAlign:"left" as const,display:"flex",alignItems:"center",gap:6}}>
          ...
          <div style={{marginBottom:12}}>
            <div style={{fontSize:9,fontWeight:800,letterSpacing:2,color:status==="RESOLVED"?"#22C55E":"#2a2a2a",marginBottom:6}}>
              {status==="RESOLVED"?"RESOLUTION NOTE · REQUIRED":"ADMIN NOTE"}
            </div>
            <textarea value={note} onChange={e=>setNote(e.target.value)} rows={3}
              placeholder={status==="RESOLVED"?"What was fixed, by whom — citizens will see this":"Internal note…"}
          ...
          {acts.length>0&&(
            <div>
              ...
              {acts.map((a:any)=>(
                <div key={a.id} style={{display:"flex",gap:8,alignItems:"flex-start",marginBottom:4}}>
                  <div style={{width:6,height:6,borderRadius:"50%",background:"#1e1e1e",flexShrink:0,marginTop:4}}/>
                  <div style={{flex:1}}>
                    <div style={{color:"#333",fontSize:11}}>{a.action==="STATUS_CHANGED"?a.detail:a.action}{a.admin&&<span style={{color:"#1a1a1a"}}> · {a.admin.name}</span>}</div>
                    <div style={{color:"#1a1a1a",fontSize:10}}>{new Date(a.createdAt).toLocaleString("en-GB",{day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"})}</div>
                  </div>
                </div>
              ))}
            </div>
...
function dlCSV(rows: any[]) {
  const hdrs  = ["ID","Date","Address","Landmark","Latitude","Longitude","Hazard","Severity","Status","Upvotes","Reporter","Description","Resolution Note","Fixed By","Photo URL"];
  const lines = [hdrs.join(","),...rows.map(r=>[r.id,new Date(r.createdAt).toLocaleDateString("en-GB"),r.address,r.landmark||"",r.latitude,r.longitude,hMeta(r.hazardType).label,SEV[r.severity]?.label||r.severity,ST[r.status]?.label||r.status,r.upvoteCount||0,r.reporter||"Anonymous",r.description||"",r.resolutionNote||"",r.fixedBy||"",r.photoUrl||""].map(esc).join(","))];
...
export default function DashboardPage() {
  ...
  useEffect(()=>{
    fetch("/api/reports").then(r=>r.json()).then(j=>{ if(j.success) setReports(j.data); }).finally(()=>setLoading(false));
  ...
  const onUpdate     = (u: any) => setReports(p=>p.map(r=>r.id===u.id?{...u,upvoteCount:r.upvoteCount}:r));
  ...
  const hasFilter     = regionFilter!=="ALL"||sevFilter!=="ALL";
  ...
  const topAlert = critical>0
    ...
    : unverified>0
    ? {msg:`${unverified} unverified · needs review`,c:"#64748B",bg:"rgba(100,116,139,0.05)",b:"rgba(100,116,139,0.1)"}
    : null;
    ...
    <div style={{background:"#0A0A0A",minHeight:"100vh",fontFamily:"'Inter',-apple-system,sans-serif",color:"#fff",paddingBottom:80}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}button{cursor:pointer;-webkit-tap-highlight-color:transparent}
        ...
        select,input[type="datetime-local"]{-webkit-appearance:none}
      `}</style>
      ...
      <div style={{padding:"14px 18px 0",display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:7}}>
        {[{v:reports.length,l:"Reports",c:"#555"},{v:pending,l:"Pending",c:"#F59E0B"},{v:critical,l:"Critical",c:"#EF4444"},{v:resolved,l:"Resolved",c:"#22C55E"}].map(s=>(
          <div key={s.l} style={{background:"#0C0C0C",border:"1px solid #111",borderRadius:12,padding:"11px 10px",textAlign:"center" as const}}>
            <div style={{color:s.c,fontSize:9,fontWeight:800,letterSpacing:.5,marginBottom:3}}>{s.l.toUpperCase()}</div>
            <div style={{color:"#fff",fontSize:22,fontWeight:900,lineHeight:1}}>{s.v}</div>
          </div>
        ))}
      </div>

      {loading&&<div style={{textAlign:"center",padding:"60px 0",color:"#2a2a2a",fontSize:14}}>Loading…</div>}
      ...
      <div style={{padding:"12px 18px 0"}}>
        ...
        {!loading&&tab==="feed"&&(
          <>
            ...
            <div style={{display:"flex",gap:7,marginBottom:8}}>
              <div style={{flex:1,background:"#0C0C0C",border:"1px solid #111",borderRadius:11,padding:"10px 13px",display:"flex",alignItems:"center",gap:7}}>
              ...
              <button onClick={()=>setShowFilter(f=>!f)}
                style={{background:hasFilter?"rgba(239,68,68,0.12)":"#0C0C0C",border:`1px solid ${hasFilter?"rgba(239,68,68,0.3)":"#111"}`,borderRadius:11,padding:"10px 14px",color:hasFilter?"#EF4444":"#444",fontSize:13,fontFamily:"inherit",fontWeight:700}}>
                ⚙{hasFilter?" ·":""}
              </button>
            </div>

            {/* Collapsible region + severity panel */}
            {showFilter&&(
              <div style={{background:"#0C0C0C",border:"1px solid #111",borderRadius:12,padding:"12px 13px",marginBottom:8}}>
                <div style={{fontSize:9,fontWeight:800,letterSpacing:1.5,color:"#2a2a2a",marginBottom:7}}>REGION</div>
                <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:4,marginBottom:10}}>
                  {activeRegions.map(r=>(
                    <button key={r} onClick={()=>setRegionFilter(r)}
                      style={{flexShrink:0,background:regionFilter===r?"#EF4444":"#0A0A0A",border:`1px solid ${regionFilter===r?"#EF4444":"#141414"}`,borderRadius:20,padding:"5px 12px",color:regionFilter===r?"#fff":"#2a2a2a",fontSize:10,fontWeight:700,fontFamily:"inherit",whiteSpace:"nowrap" as const}}>
                      {r==="ALL"?"All Regions":r}
                    </button>
                  ))}
                </div>
                <div style={{fontSize:9,fontWeight:800,letterSpacing:1.5,color:"#2a2a2a",marginBottom:7}}>SEVERITY</div>
                <div style={{display:"flex",gap:5,flexWrap:"wrap" as const}}>
                  {[{k:"ALL",l:"Any"},{k:"CRITICAL",l:"Critical"},{k:"HIGH",l:"High"},{k:"MEDIUM",l:"Moderate"},{k:"LOW",l:"Low"}].map(s=>(
                    <button key={s.k} onClick={()=>setSevFilter(s.k)}
                      style={{background:sevFilter===s.k?"rgba(239,68,68,0.15)":"#0A0A0A",border:`1px solid ${sevFilter===s.k?"rgba(239,68,68,0.35)":"#141414"}`,borderRadius:20,padding:"5px 10px",color:sevFilter===s.k?"#EF4444":"#2a2a2a",fontSize:10,fontWeight:700,fontFamily:"inherit"}}>
                      {s.l}
                    </button>
                  ))}
                </div>
                {hasFilter&&(
                  <button onClick={()=>{setRegionFilter("ALL");setSevFilter("ALL");}} style={{marginTop:10,background:"none",border:"none",color:"#EF4444",fontSize:11,fontWeight:700,fontFamily:"inherit",padding:0}}>
                    Clear filters
                  </button>
                )}
              </div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
              <span style={{color:"#222",fontSize:9,fontWeight:700,letterSpacing:.5}}>{filtered.length} REPORTS</span>
        ...
        {!loading&&tab==="map"&&(
          <>
            ...
            <div style={{borderRadius:16,overflow:"hidden",marginBottom:10,border:"1px solid #111",height:340}}>
          ...
          <>
            <div style={{background:"#0C0C0C",border:"1px solid #111",borderRadius:14,padding:"16px",marginBottom:12}}>
              ...
              <input value={aForm.title} onChange={e=>setAForm(f=>({...f,title:e.target.value}))} placeholder="Title"
                style={{width:"100%",background:"#0A0A0A",border:"1px solid #141414",borderRadius:10,padding:"11px 13px",color:"#fff",fontSize:14,fontFamily:"inherit",marginBottom:8,outline:"none"}}/>
              <textarea value={aForm.body} onChange={e=>setAForm(f=>({...f,body:e.target.value}))} placeholder="Message to citizens…" rows={3}
                style={{width:"100%",background:"#0A0A0A",border:"1px solid #141414",borderRadius:10,padding:"11px 13px",color:"#fff",fontSize:13,fontFamily:"inherit",resize:"none" as const,marginBottom:8,outline:"none"}}/>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
                <div>
                  <div style={{fontSize:9,fontWeight:800,letterSpacing:1.5,color:"#2a2a2a",marginBottom:5}}>REGION</div>
                  <select value={aForm.region} onChange={e=>setAForm(f=>({...f,region:e.target.value}))}
                    style={{width:"100%",background:"#0A0A0A",border:"1px solid #141414",borderRadius:10,padding:"10px 12px",color:aForm.region?"#fff":"#333",fontSize:13,fontFamily:"inherit",outline:"none"}}>
                    <option value="">National</option>
                    {GH_REGIONS.map(r=><option key={r} value={r}>{r}</option>)}
                  </select>
                </div>
                <div>
                  <div style={{fontSize:9,fontWeight:800,letterSpacing:1.5,color:"#2a2a2a",marginBottom:5}}>EXPIRES</div>
                  <input type="datetime-local" value={aForm.expiresAt} onChange={e=>setAForm(f=>({...f,expiresAt:e.target.value}))}
                    style={{width:"100%",background:"#0A0A0A",border:"1px solid #141414",borderRadius:10,padding:"10px 12px",color:aForm.expiresAt?"#fff":"#333",fontSize:13,fontFamily:"inherit",outline:"none"}}/>
                </div>
              </div>
        ...
        {!loading&&tab==="analytics"&&(()=>{
          ...
          const byArea = Object.entries(
            reports.filter(r=>r.areaId).reduce((acc:any,r)=>{ acc[r.areaId]=(acc[r.areaId]||{count:0}); acc[r.areaId].count++; return acc; },{})
          ...
          const byHazard = H.map(h=>({...h,count:reports.filter(r=>r.hazardType===h.key&&r.status!=="DISMISSED").length})).sort((a,b)=>b.count-a.count);
          const maxH     = Math.max(...byHazard.map(x=>x.count),1);

          const resolvedReps   = reports.filter(r=>r.status==="RESOLVED");
          const resRate        = reports.length?Math.round((resolvedReps.length/reports.length)*100):0;
          const positiveTimes  = resolvedReps
            .filter(r=>r.resolvedAt)
            .map(r=>(new Date(r.resolvedAt).getTime()-new Date(r.createdAt).getTime())/3600000)
            .filter(t=>t>0);
          const avgHrs = positiveTimes.length
            ? Math.round(positiveTimes.reduce((s,t)=>s+t,0)/positiveTimes.length*10)/10
            : null;
                ...
                <div style={{background:"#0C0C0C",border:"1px solid #111",borderRadius:13,padding:"14px",marginBottom:8}}>
                  ...
                  {byArea.map(([id,{count}]:any,i)=>(
                    <div key={id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                      ...
                      <div style={{flex:1}}>
                        <div style={{color:"#555",fontSize:12,marginBottom:2}}>{areaName(id)}</div>
      ...
      <div style={{position:"fixed" as const,bottom:0,left:0,right:0,background:"rgba(5,5,5,0.97)",borderTop:"1px solid #0F0F0F",padding:"9px 0 20px",display:"flex",justifyContent:"space-around",backdropFilter:"blur(20px)",zIndex:99}}>
        {[["feed","📋","Feed"],["map","🗺️","Map"],["announce","📢","Alerts"],["analytics","📊","Stats"]].map(([key,icon,label])=>(
          <button key={key} onClick={()=>setTab(key)} style={{background:"none",border:"none",display:"flex",flexDirection:"column" as const,alignItems:"center",gap:2,fontFamily:"inherit",minWidth:60,position:"relative" as const}}>
            ...
            {key==="announce"&&announcements.length>0&&(
              <span style={{position:"absolute" as const,top:0,right:10,width:6,height:6,borderRadius:"50%",background:"#EF4444"}}/>
            )}
          </button>
        ))}
      </div>
